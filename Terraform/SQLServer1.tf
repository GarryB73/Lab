...
#
resource "vsphere_virtual_machine" "SQLServer1" {
  name                 = "${var.SQLServer1_name}"
  num_cpus             = "${var.SQLServer1_cpu_num}"
  memory               = "${var.SQLServer1_mem}"
...
#
  ...
  clone {
    ...
    customize {
      windows_options {
        computer_name    = "${var.SQLServer1_name}"
